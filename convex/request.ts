...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.email === identity.email) {
      throw new ConvexError("Can't send a message to yourself");
    }
    ...
    const requestAlreadySent = await ctx.db
      ...
      .unique();
    console.log(receiver._id, currentUser._id)
    ...
    if (requestAlreadyReceived) {
      throw new ConvexError("This user has already sent you a message");
    }
...
export const deny = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!request || request.receiver !== currentUser._id) {
      throw new ConvexError("The was an error denying this request")
    }
